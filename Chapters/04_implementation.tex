...
In this subsection, we discuss the most foundational design choices for this feature.
...
Whenever a variable is moved (which is in the language patters \code{Assign(_, Path(_))}, \code{LetInit(_, Path(_))} and \code{FieldInit(_, Path(_)})